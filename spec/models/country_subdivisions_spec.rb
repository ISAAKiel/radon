...
describe "CountrySubdivisionSubdivisions" do
  it "has a valid factory" do
    expect(FactoryBot.build(:country_subdivision)).to be_valid
  end
  it "should act_as_list" do
    expect(FactoryBot.build(:country_subdivision)).to respond_to(:move_to_top)
  end
  ...
  it "is invalid without :name" do
    expect(FactoryBot.build(:country_subdivision, name: nil)).not_to be_valid
  end

  it "does allow duplicate :name at different countries" do
    FactoryBot.create(:country_subdivision, name: "1", country_id: "1")
    expect(FactoryBot.build(:country_subdivision, name: "1", country_id: "2")).to be_valid
  end

  it "does not allow duplicate :name at the same country" do
    FactoryBot.create(:country_subdivision, name: "1", country_id: "1")
    expect(FactoryBot.build(:country_subdivision, name: "1", country_id: "1")).not_to be_valid
  end
end
